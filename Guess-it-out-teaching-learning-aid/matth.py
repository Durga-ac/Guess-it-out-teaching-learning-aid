...
def hlvl():
    ...
    def start_game(args):
        if args == 3:
            lvll.destroy()
            from Options import level_7
            level_7.main()
        elif args == 4:
            from Options.level_7 import points
            if points < 10:
                messagebox.showerror("Error", 'Score at least 10 to Unlock level 2')
            else:
                messagebox.showinfo('Success', 'Level Unlocked')
                lvll.destroy()
                from Options import level_8
                level_8.main()
